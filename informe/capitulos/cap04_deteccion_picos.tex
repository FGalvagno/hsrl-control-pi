...
Por razones de costo e implementación, se adoptó la segunda alternativa, ya que permite emplear un ADC de menor velocidad de muestreo y costo reducido, disminuyendo simultáneamente el volumen de datos a procesar por la unidad microcontroladora.
...
\begin{table}[h]
  ...
  \begin{tabular}{lll}
    ...
    \bottomrule
  \end{tabular}
  \caption{Características principales del amplificador realimentado por corriente AD8004 \cite{ad8004_ds}.}
...
\begin{table}[h]
  ...
  \begin{tabular}{lll}
    ...
    \bottomrule
  \end{tabular}
  \caption{Características principales del transistor bipolar NPN BC817 \cite{bc817_ds}.}
...
\begin{itemize}
  ...
  \item La constante de tiempo $\tau = R_3 C_1$ debe garantizar la aproximación lineal descrita en la Ecuación \ref{eq:carga_rc_lineal} durante el pulso de entrada.
  \item La resistencia $R_3$ debe limitar la corriente de pico para no superar el límite absoluto de $I_C$ en el transistor $Q_1$.
...
A partir de las caracterizaciones experimentales de la señal emitida por los fotodetectores, se determinó un tiempo de subida de pulso de aproximadamente $20\,\text{ns}$. Para cumplir con la condición $t \ll R_3 C_1$, se fijó un valor objetivo $\tau = R_3 C_1 \approx 20\,\mu\text{s}$.

La resistencia $R_3$ limita la corriente pico impulsiva provista por el transistor. Considerando una corriente máxima admisible $I_{C,\text{pico}} = 1\,\text{A}$ ante pulsos ultra-cortos, se tiene:

\begin{equation}
  R_3 = \frac{V_{O,\text{U1A}} - V_{BE}}{I_{C,\text{pico}}} = \frac{12\,\text{V} - 1.2\,\text{V}}{1\,\text{A}} = 10.8\,\Omega
  \label{eq:valor_r3_calc}
\end{equation}

Seleccionando un valor comercial E24 estándar de $R_3 = 8.2\,\Omega$ junto con un capacitor $C_1 = 3.3\,\text{nF}$, se obtiene una constante de tiempo efectiva de:

\begin{equation}
  \tau = R_3 C_1 = 8.2\,\Omega \times 3.3\,\text{nF} = 27.06\,\mu\text{s}
  \label{eq:tau_final}
\end{equation}

\noindent expresión que satisface rigurosamente la condición de linealidad establecida en la Ecuación \ref{eq:carga_rc_lineal}.
...
\begin{table}[h]
  ...
  \begin{tabular}{c c c c l}
    ...
    \bottomrule
  \end{tabular}
  \caption{Componentes pasivos seleccionados para el circuito detector de picos.}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{figuras/4_señalpulso.png}
...
Se fabricó un primer prototipo en laboratorio mediante la técnica de transferencia térmica y grabado químico con cloruro férrico. Se montaron componentes de tecnología de inserción superficial (SMD) para minimizar el impacto de las componentes parásitas.
...
\begin{figure}[htbp]
  ...
  \caption{Respuesta del prototipo ante un pulso de $100\,\text{ns}$ ($20\,\text{Hz}$), evidenciando oscilaciones de alta frecuencia derivadas de parásitos del trazado\protect\footnotemark[1] (Fuente: Elaboración propia).}
  \label{fig:oscilaciones}
\end{figure}

\footnotetext[1]{Debido a un error en la configuración del buffer de captura del osciloscopio, la nitidez de la Figura \ref{fig:oscilaciones} fue mejorada computacionalmente mediante algoritmos de inteligencia artificial, usando el modelo de Google Nano Banana Pro y como prompt "reduce el ruido de fondo de esta imagen preservando fielmente la forma de la onda y los valores capturados".}
